...
title: "测试右键菜单",
onclick: function(){aler　t('您点击了右键菜单！');}
});
```
...
{
aler　t(JSON.stringify(resources));
});
});
```
...
bg.test(); // 访问bg的函数
aler　t(bg.document.body.innerHTML); // 访问bg的DOM
```
...
if(request.cmd == 'test') aler　t(request.value);
...
```javascript
$('#btn').on('click', function(){aler　t('测试')});
```
